...
-- User-facing cash-flow totals are keyed by dividend_events.payment_year.

alter table public.dividend_events
  add column if not exists payment_year int;

update public.dividend_events
set payment_year = coalesce(
  payment_year,
...
alter table public.user_settings
  add column if not exists annual_dividend_goal_amount numeric(18,2);

create index if not exists idx_dividend_events_stock_payment_year
  on public.dividend_events(stock_id, payment_year);

create index if not exists idx_dividend_events_payment_year_month
  on public.dividend_events(payment_year, expected_payment_month);

create or replace function public.set_dividend_event_payment_year()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  if new.payment_year is null then
    new.payment_year := coalesce(
      extract(year from new.expected_payment_date)::int,
      new.fiscal_year
    );
  end if;
...
create trigger set_dividend_events_payment_year
before insert or update on public.dividend_events
for each row execute function public.set_dividend_event_payment_year();
