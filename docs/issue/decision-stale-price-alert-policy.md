...
2. **평가 중단 대상**: 수익률 기반 알림 룰만 중단
   - `yield_above`
   - `yield_below`
   - `after_tax_yield_above`
   - `after_tax_yield_below`
3. **평가 계속 대상**: 주가와 무관한 알림 룰은 정상 평가
   ...
   - `special_dividend`
   - `ex_date_before`
4. **중단 시 로그**: 스킵된 평가는 낶부 로그에 기록 (디버깅용)
...
- 사용자는 알림을 "놓친" 것으로 인지할 수 있으나, 이는 잘못된 알림을 본내는 것보다 안전합니다.
- 3일 연속 API 실패 시 관리자에게 알림이 가므로, 관리자는 문제를 인지하고 조치할 수 있습니다.

## 근거
- 금융 알림 서비스에서 **잘못된 알림을 본내는 것보다 알림을 본내지 않는 것이 훨씬 안전**합니다.
